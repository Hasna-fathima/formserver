...
const getEmployeeSortedByName=async(req,res)=>{
    try{
const records=await employeemodel.aggregate([
    {$sort:{name:-1}},
    {$limit:4},
    
    {$project:{_id:0,EmployeeName:"$name",rank:1}}
])
res.status(200).json(records)
...
module.exports={AddEmployees,countByAge,countByAgeGreaterThan,getEmployeeSortedByName}